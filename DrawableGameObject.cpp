...
void DrawableGameObject::setPosition(Vector2D position)
{
	m_position = XMFLOAT3((float)position.x, (float)position.y, 0.0f);
}

Vector2D DrawableGameObject::getPosition()
{
	return Vector2D(m_position.x, m_position.y);
}

void DrawableGameObject::setXMPosition(XMFLOAT3 position)
{
	m_position = position;
}
